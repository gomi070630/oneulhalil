...
const MockContext = reactExports.createContext(null);
...
function MockProvider({ children }) {
  const [categories, setCategories] = reactExports.useState(
    () => loadFromStorage(CATEGORIES_KEY, INITIAL_CATEGORIES)
  );
  const [tasks, setTasks] = reactExports.useState(
    () => loadFromStorage(TASKS_KEY, INITIAL_TASKS)
  );
  const [routines, setRoutines] = reactExports.useState(
    () => loadFromStorage(ROUTINES_KEY, INITIAL_ROUTINES)
  );
...
function useMockStore() {
  const ctx = reactExports.useContext(MockContext);
